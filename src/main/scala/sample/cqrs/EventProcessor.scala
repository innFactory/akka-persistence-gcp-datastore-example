...
object EventProcessor {
  ...
  def init[Event](
    ...
    eventProcessorStream: String => EventProcessorStream[Event]
  ): Unit = {
    val eventProcessorEntityKey = entityKey(settings.id)

    ClusterSharding(system).init(
      Entity(eventProcessorEntityKey)(entityContext =>
        EventProcessor(eventProcessorStream(entityContext.entityId), None)
      ).withRole("read-model")
    )
  ...
  def apply(eventProcessorStream: EventProcessorStream[_], id: Option[String]): Behavior[Ping.type] =
    Behaviors.setup { context =>
      ...
      eventProcessorStream match {
        ...
        case persistence: ShoppingCartPersistenceIdEventProcessorStream =>
          eventProcessorStream.runQueryStreamForPersistenceId(killSwitch, id.getOrElse(""))
        case ids: PersistenceIdsEventProcessorStream => eventProcessorStream.runQueryStreamForPersistenceIds(killSwitch)
      }

      Behaviors
        .receiveMessage[Ping.type](ping => Behaviors.same)
        .receiveSignal {
          case (_, PostStop) =>
            killSwitch.shutdown()
            Behaviors.same
  ...
  private def processEventsByPersistenceId(persistenceId: String): Source[Long, NotUsed] =
    query.eventsByPersistenceId(persistenceId, -1, 2).mapAsync(1) { eventEnvelope =>
      eventEnvelope.event match {
        case event: Event =>
          processEvent(event, PersistenceId.ofUniqueId(eventEnvelope.persistenceId), eventEnvelope.sequenceNr)
            .map(_ => eventEnvelope.sequenceNr)
        case other =>
          Future.failed(new IllegalArgumentException(s"Unexpected event [${other.getClass.getName}]"))
      }
    }
