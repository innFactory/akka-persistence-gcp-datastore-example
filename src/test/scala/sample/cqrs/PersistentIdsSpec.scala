...
class PersistentIdsSpec extends ScalaTestWithActorTestKit(ConfigFactory.parseString(s"""
  ...
  "The PersistentIds" should {
    ...
    "be consumed by the event processor" in {
      ...
      testKit.spawn(
        EventProcessor(new PersistenceIdsEventProcessorStream(system, system.executionContext, "EventProcessor", "ShoppingCart|cart-1-proc"), Some("ShoppingCart|cart-1-proc")))
