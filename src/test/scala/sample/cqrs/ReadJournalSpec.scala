...
class ReadJournalSpec extends ScalaTestWithActorTestKit(ConfigFactory.parseString(s"""
  ...
  "The events from the Shopping Cart" should {
    "be consumed by the read journal processor for persistenceId" in {
      ...
      testKit.spawn(
        EventProcessor(
          new ShoppingCartPersistenceIdEventProcessorStream(
            system,
    ...
    "be not consumed by the read journal processor for other persistenceId" in {
      ...
      testKit.spawn(
        EventProcessor(
          new ShoppingCartPersistenceIdEventProcessorStream(
            ...
            system.executionContext,
            "EventProcessor",
            "ShoppingCart|cart-2-proc"
          ),
          Some("ShoppingCart|cart-2-proc")
        )
      )
      ...
      eventProbe.expectMessage(
        max = FiniteDuration.apply(15, duration.SECONDS),
        ShoppingCart.ItemAdded("cart-2-proc", "foo", 42)
      )
      ...
      eventProbe.expectMessage(
        max = FiniteDuration.apply(15, duration.SECONDS),
        ShoppingCart.ItemAdded("cart-2-proc", "bar", 17)
      )
      cart1 ! ShoppingCart.AdjustItemQuantity("bar", 18, probe.ref)
      probe.expectMessageType[ShoppingCart.Accepted]
      eventProbe.expectMessage(
        max = FiniteDuration.apply(15, duration.SECONDS),
        ShoppingCart.ItemQuantityAdjusted("cart-2-proc", "bar", 18)
      )
